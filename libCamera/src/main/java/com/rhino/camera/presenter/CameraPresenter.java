package com.rhino.camera.presenter;

import android.content.Context;
import android.content.pm.PackageManager;

import com.rhino.camera.camera2.BaseCamera2Operator;
import com.rhino.camera.camera2.Camera2Manager;
import com.rhino.camera.camera2.utils.Camera2Utils;
import com.rhino.camera.contract.CameraContract;
import com.rhino.camera.mode.Constant;
import com.rhino.camera.utils.permissions.PermissionsManager;
import com.rhino.camera.utils.thread.WorkThreadUtils;
import com.rhino.camera.utils.time.TimingUtils;
import com.rhino.camera.utils.toast.ToastUtils;
...
public class CameraPresenter implements CameraContract.Presenter, BaseCamera2Operator.Camera2ResultCallBack, BaseCamera2Operator.Camera2VideoRecordCallBack {
    ...
    public CameraPresenter(CameraContract.View view) {
        ...
        this.appContext = view.getCameraView().getContext();
    ...
    public void switchMode(int mode) {
        ...
        switch (currentMode) {
            ...
            case Constant.MODE_CAMERA:
                view.showToast("请稍等，正在切换到拍照模式");
                break;
            //切换到录像模式
            case Constant.MODE_VIDEO_RECORD:
                view.showToast("请稍等，正在切换到录像模式");
                break;
            default:
                break;
        }
    ...
    @Override
    public void switchCamera1(int direction) {
        this.camera2Manager.switchCameraDirection1(direction);
    }

    @Override
    public int getCameraDirection() {
        return camera2Manager.getCurrentDirection();
    }
        ...
        if (result != null) {
            Subscription subscription = result.subscribeOn(Schedulers.io())
                    ...
                    .subscribe(filePath -> {
                            ...
                            , throwable -> {
                                //写入图片到磁盘失败
                                ToastUtils.showToast(appContext, "写入磁盘失败");
                            });
        ...
        switch (requestCode) {
                ...
                if (grantResults.length == PermissionsManager.CAMERA_REQUEST.length) {
                    for (int result : grantResults) {
                        if (result != PackageManager.PERMISSION_GRANTED) {
                            ToastUtils.showToast(appContext, "拍照权限被拒绝");
                            break;
                        }
                    }
                }
            ...
            case PermissionsManager.VIDEO_REQUEST_CODE:
                if (grantResults.length == PermissionsManager.VIDEO_PERMISSIONS.length) {
                    for (int result : grantResults) {
                        if (result != PackageManager.PERMISSION_GRANTED) {
                            ToastUtils.showToast(appContext, "录像权限被拒绝");
                            break;
                        }
                    }
                }
    ...
    @Override
    public void setCameraVideoOutputDirectory(String directoryPath) {
        camera2Manager.setCameraVideoOutputDirectory(directoryPath);
    }

    @Override
    public void setCameraPictureOutputDirectory(String directoryPath) {
        camera2Manager.setCameraPictureOutputDirectory(directoryPath);
    }
